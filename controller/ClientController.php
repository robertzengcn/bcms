    ...
    public function save() {     	 	  	
    	if(!isset($_REQUEST['id'])||$_REQUEST['id']==''){
    		...
    		if($result->statu){//如果保存用户成功，则开始保存客服记录
				...
				$info = array('reception'=>$reception,'type'=>1,'source_id'=>$_REQUEST['source'],'region_id'=>$_REQUEST['region_id']);
				$time = strtotime($_REQUEST['record'][0]['recordtime']);
				...
				if($resdata){
					...
					$info['region']   = $resdata->region;
					$info['source'] = $resdata->source;
					//$info['department'] = $resdata->department;
					$info['kefu'] = $resdata->kefu;
					$this->service->updateClientData($cdata,$info,1);
				}else{
					$this->service->saveClientData($cdata,$info,1);						
				}
...
//         }
		...
		$recordCount = $this->service->getTotalRecord(intval($_REQUEST['id']));
		$recordUpdateCount = count($_REQUEST['record']) - $recordCount;
		$recordUpdateCount = ($recordUpdateCount>=0) ? $recordUpdateCount : 0;
		$reception = array();
		if(isset($_REQUEST['record'][0]['reception_id'])){
			$reception[$_REQUEST['record'][0]['reception_id']]	= $recordUpdateCount;
		}elseif(isset($_REQUEST['reception_id'])){
			$reception[$_REQUEST['reception_id']]	= $recordUpdateCount;		
		}
        $clientRecords = array();
        if($contents){      
        ...
        if($result->statu){//如果保存用户成功，则开始保存客服记录
        	...
        	if (isset($_REQUEST['focus_id']) && !empty($_REQUEST['focus_id'])) {
        		...
        		foreach($_REQUEST['focus_id'] as $val){
 				...
 				$info = array('reception'=>$reception,'type'=>1,'source_id'=>$_REQUEST['source']);
				$time = time();
				$resdata = $this->service->checkData($time);
					$this->blindParams($cdata = new ClientData());
					$cdata->week = date("w",$time);
				...
				if($resdata){
					$cdata->id = $resdata->id;
					$cdata->zx_total = $resdata->zx_total + $recordUpdateCount;
					$cdata->res_total = $resdata->res_total;
					$cdata->region = $resdata->region;
					$info['source'] = $resdata->source;
					//$info['department'] = $resdata->department;
					$info['kefu'] = $resdata->kefu;
					$this->service->updateClientData($cdata,$info,2);
				}else{
					$this->service->saveClientData($cdata,$info,2);							
				}       	
...
// 					}
 					if($results['statu']){
						$info = array('department_id'=>$_REQUEST['department_id'],'source_id'=>$_REQUEST['source'],'statu'=>$_REQUEST['statu'],'type'=>2);
						$time = $_REQUEST['date'];
						$resdata = $this->service->checkData($time);
							$this->blindParams($cdata = new ClientData());
							$cdata->week = date("w",$time);
    ...
    public function updateStatData(){
		$start = isset($_REQUEST['start']) ? strtotime($_REQUEST['start'] . ' 00:00:59') : '';
		$end = isset($_REQUEST['end']) ? strtotime($_REQUEST['end'] . ' 23:59:59') : '';
		$result = $this->service->updateStatData($start, $end);        
    	echo json_encode($result);
	}
}

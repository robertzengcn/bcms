...
class ResVationController extends Controller {
    ...
    public function addResvation(){    	
    	...
    	$_REQUEST['mark']=isset($_REQUEST['mark'])&&strlen($_REQUEST['mark'])>0?$_REQUEST['mark']:0;  	
    	$_REQUEST['date']=strtotime($_REQUEST['date']);
...
//     exit();
     ...
     * 专家出诊表(一周)
     */	
    public function queryDateList() { 
		$timestr = time();
		$now_day = date('w',$timestr);
		$where['start_date'] = $timestr - $now_day*60*60*24;		//获取一周的第一天，星期天
		$where['end_date'] = $timestr + (6-$now_day)*60*60*24; //获取一周的最后一天，星期六
		$resdepartmentdao = new ResDepartmentDAO();
		$resdepartment = $resdepartmentdao->getResDepartmentName();	
        $array=array();
		if($resdepartment){
			$i = 0;
			foreach ($resdepartment as $key=>$value){
				$time = $this->service->queryDateList($where,$value['id']);
				if($time['isnull']==1){
					$array['data'][$i] = array('name'=>$value['name'],'time'=>$time);
					$i++;
				}
			}
			unset($resdepartment);
    ...
    public function getStatData() {
        ...
        $total = isset($rows['total']) ? $rows['total'] : '';
        $rows = isset($rows['rows']) ? $rows['rows'] : $rows;
        echo json_encode(array('rows'=>$rows,'ttl'=>$ttl,'total'=>$total));
    }
    /**
     * 更新统计数据
     * */

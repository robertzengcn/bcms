...
//		$this->makeOnce(true);
	...
	try{
		...
		$time=time();
		$error_log=GENERATEPATH . 'log/mod_error_'.$time.'.log';
		error_log(date('Y-m-d H:i:s') . ' mod error:' . $e->getMessage() . PHP_EOL, 3, $error_log);
	}
	}
...
/* 		$smartyplugin=array(PLUGINSDIR_SMARTY_SECOND);
	...
	public function makeOnce() {
		...
		$this->smarty->assign('TEMPPATH', TEMPDIR . $this->tag->templateName);//本次模版路径
		$this->smarty->assign('MODULEKEY', strtolower($this->config['key']));//模版key
	}
	...
	public function msgPut($status,$code,$msg,$data=null){
		die( json_encode( array('statu'=>$status,'code'=>$code,'msg'=>$msg,'data'=>$data) ) );
	}

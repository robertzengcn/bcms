...
function smarty_block_doctorlist( $params, $content, $template, &$repeat ) {
    ...
    $dmjoin = isset($params['department']) ? $params['department'] : '';    
    ...
    if (!$repeat) {
        ...
        foreach ($data as $key=>$doctorinfo) {
            ...
            if($dmjoin){
				$dem = new DepartmentTag();	
				$dm_data = $dem->getInfo($doctorinfo->department_id,$dmjoin);
				foreach($dm_data as $k=>$val){
					$dm_name = 'department_'. $k;
					if($val == 'pic'){
						$data[$key]->$dm_name = rtrim(UPLOAD, '/') . $val;
					}else{
						$data[$key]->$dm_name = $val;						
					}
				}				
			}
...
function smarty_block_doctorsbydept($params, $content, $template, &$repeat){
    ...
    if (!$repeat) {
        ...
        foreach ($data as $k=>$docinfo) {
            if ($docinfo->pic&&strpos($docinfo->pic, NETADDRESS) === false) {
                $data[$k]->pic = UPLOAD . $docinfo->pic;
            }
            
            $url = getClientUrl('doctor', $docinfo->id);
            $data[$k]->url = $url ? $url : NETADDRESS . "/doctor/" . $docinfo->id . ".html";
        }

...
var validate = {
		...
		'hometel':{c:true,m:'请正确填写联系方式',l:{min:0,max:0},v:{min:0,max:0},p:/^[0-9]{11}$/},
		...
		'time':{c:true,m:'预约门诊未选择',l:{min:1,max:11},v:{min:0,max:0},p:""},		
...
Reser.fromValidate = function(formID){
	...
	$.each(validate,function(key,value){
		...
		//根据用户需求确定是否需要验证
		//var check = value.c;
		var check = selectOb.attr('check');
		if (selectOb.attr('check') == undefined || selectOb.attr('check') != 'true') {
			check = 'false';
		}
		switch(check){
		     case 'true' :

...
function Ask() {
		...
		self.cmd(gHttp,{controller:'Ask',method:'getRepeat',id:id},function(result1){
			if(result1.statu){
				...
				if( result1.data.pic1 != '' && result1.data.pic1 != null ){
					$('#picboxs').html('<img src="'+result1.data.pic1+'">');
					//$('#ask_pic_pic').attr('src',result1.data.pic1);
				}
				if( result1.data.content != '' ) {
					$('#addToHtmlHeader').html( "RE："+result1.data.content );
				}
				if( result1.data.assay_pic == '' ) {
					$('#assay_box').html('该用户未上传报告单.');
				}else{
					$('#assay_box').html('<img src="'+result1.data.pic1+'">');
	...
	this.checkplus=function(){
		self.cmd(gHttp,{controller:'Contact',method:'getContacts',flag:'jpushstate'},function(ret){
			if(ret.statu){
                   if(ret.data.val){
                	   $('#plusdiv').show();  
                   }else{

...
class PluginController extends Controller {
	...
	public function getRemotePluginList() {
		...
		if(count($list) <= 0){
			...
			if(count($list) >= 1){
				...
				//循环递补数据
				foreach($list['data'] as $key => $value){
				    if (empty($list['data'][$key]['update_time'])) {
				        $list['data'][$key]['update_time'] = date('Y-m-d',$list['data'][$key]['add_time']);
				    } else {
	...
	/**
	 * (远程)获取业界咨询
	 * @method getRemoteHomeAdvert
	 */
	public function getRemoteHomeAdvert() {
	    $url = 'http://www.boyicms.com/interface/hwibs/advert/advert.php';
	    $content = @Io::urlPost( $url . '?domain='.$_SERVER['SERVER_NAME'] . '&position=homeAdvert' );
	    //外部加了一层()，所以要去掉才能成功解析
	    $content = substr($content, 1, -1);
	    $advert = json_decode($content, true);
		if($advert){
			die( json_encode( $advert ) );
		}else{
	...
	/**
	 * (远程)获取新闻阅读
	 * @method getRemoteHomeNews
	 */
	public function getRemoteHomeNews() {
	    $url = 'http://www.boyicms.com/interface/hwibs/advert/news.php';
	    $content = @Io::urlPost( $url . '?domain='.$_SERVER['SERVER_NAME'] . '&position=homeNews' );
	    //外部加了一层()，所以要去掉才能成功解析
	    $content = substr($content, 1, -1);

...
function smarty_block_arclist($params, $content, $template, &$repeat) {
    ...
    if (!$repeat) {
        ...
        foreach ($data as $key=>$arcinfo) {
            ...
            $url = getClientUrl('article', $arcinfo->id);
            if (!$url)           
...
function smarty_block_arcsbydept($params, $content, $template, &$repeat){
    ...
    if (!$repeat) {
        $ser = new ArticleTag();
        $data = $ser->getByDeparment($department, $page, $pagesize, $order,$show_position);
...
function smarty_block_arcsbydisease($params, $content, $template, &$repeat){
    ...
    if (!$repeat) {
        $ser = new ArticleTag();
        $data = $ser->getByDisease($department,$disease,$page,$limit,$show_position);
        foreach ($data as $k=>$v) {
            if ($v->pic&&strpos($v->pic, NETADDRESS) === false) {
                $data[$k]->pic = UPLOAD . $v->pic;
            }
            ...
            if (!$url)
                $url = $ser->getArticleUrl($v->id, false);
            $data[$k]->url = $url;

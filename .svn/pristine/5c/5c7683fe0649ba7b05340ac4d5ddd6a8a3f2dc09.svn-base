	...
	this.fillDataTable = function() {
		$("#dataTable").grid({
			...
			field : [
			            ...
			            { data: 'subject',
							render : function (id, type, row) {
			            		return '<span>'+row.subject.substr(0,18)+'...</span>';
			            	}
						},
			            { data: 'quantity'},
			            { data: 'salenum' },
			...
			if (para.id != undefined) {
				...
				self.cmd(gHttp,{controller:'Commodity',method:'get',id:para.id},function(result2){
					if(result2.statu){
						...
						if(result2.data.pic != ''){
							$('#thumbnail').attr('src', '/upload/' + result2.data.pic);
							$('#pic').val(result2.data.src);							
						}
						...
						if(result2.data.piclist && len>0){
							...
							$.each(piclist,function(i,n){
								...
								var html = '<li class=\"maskWraper\" id=\"list'+num+'\">';
									html +=	'<div class=\"bg_pic\"><img src="'+piclist[i]+'" flag="../../images/boyicms/logo/thumbnail_auto.gif\"  width=\"150px\" height=\"90px\"/></div>';
		...
		if (id == '') {
			...
			$('#formEdit').checkAndSubmit('save',post,function(result1){
				if(result1.statu){				
					...
					//$('#dataTable', window.parent.document).trigger('click');

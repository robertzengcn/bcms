...
class BoyiManagerController extends Controller {
    ...
    private $smsurl="http://www.boyicms.com/interface/sms/SMSInterface.php";
    private $smsbuyurl='http://www.boyicms.com/user/?q=sms';
    ...
    public function getmsg(){
    	...
    	if(preg_match("/^13[0-9]{1}[0-9]{8}$|15[0-9]{1}[0-9]{8}$|18[0-9]{1}[0-9]{8}$|17[0-9]{1}[0-9]{8}/",$phonenum)){ //验证通过   		
    		...
    		if($result['res']==0){
    			...
    			echo json_encode(array('stute'=>false,'msg'=>isset($result['msg'])?$result['msg']:"",'code'=>0,'data'=>null));
    		}    		    		
    	}else{
    		//手机号码格式不对
    ...
    public function checkinfo(){
    	$url=$this->smsurl."?action=getHospitalInfo" . "&" . $this->formatApiTokenQry('getHospitalInfo');

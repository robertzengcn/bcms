...
class MemberController extends Controller {
    ...
    * */
    ...
    public function get_memberscorelog(){
    	 $memberscorelog=new CommodityAddScoreLogService();
    	 $result =$memberscorelog->query($_REQUEST);

    	$totalRows = $memberscorelog->totalRows($_REQUEST);
    	echo json_encode(array(
    			'rows' => $result->data,
    			'ttl' => empty($totalRows) ? 0 : $totalRows[0]->num
    	));
    }
    ...
    * */
    public function exportdate(){
    	...
    	if(empty($alldate->data)){
    		...
    		$str = "用户id,用户名,联系电话,消费积分,消费现金,现有积分(个),消费时间\n"; 
    		//while(list($key, $value)=each($alldate->data)){
    		foreach($alldate->data as $value){
    			$str .= $value->id.",".$value->username.",".$value->telephone.",".$value->totalscore.",".$value->totalcash.",".$value->ownscore.",".$value->buytime."\n";

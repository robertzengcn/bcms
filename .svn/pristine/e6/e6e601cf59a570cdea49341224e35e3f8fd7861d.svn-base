...
//     }
	...
	public function getArticleAll(){
		...
		DTOrder::$sql = '';
		DTOrder::desc('id');
		$articles = $this->getByComposite(ORMMap::$classes[$this->tableName], DTExpression::$sql . DTOrder::$sql . DTExpression::$limit, DTExpression::delFields($this->tableName,array('content')));
	 ...
	 * @return array
     */	
    public function getAllArticleId($where) {
		if($where['did'] && $where['dis']){
			$data = R::getAll( "SELECT id FROM ".$this->tableName." WHERE 1=1 and department_id=".$where['did']." and disease_id=".$where['dis'] );
		}elseif(isset($where['begin_time']) && isset($where['end_time'])){
			$data = R::getAll( "SELECT id FROM ".$this->tableName." WHERE 1=1 and updatetime between '".$where['begin_time']."' and ".$where['end_time'] );			
		}elseif(isset($where['id'])){
			$data = R::getAll( "SELECT id FROM ".$this->tableName." WHERE 1=1 and id >=".$where['id'] );			
		}else{

 ...
 *
 */
class voteAddons extends Addons {
	...
	private function getWxUserOpenId($id) {
		...
		if (!empty($wxId)) {
			...
			if (isset($result['errcode'])) {
				$this->error('授权失败,请联系管理员。<br/>errmsg：'.$result['errmsg'].'<br/>errcode：'.$result['errcode']);
			}
...
/* 				$accessToken = $this->getAccessToken();	
	...
	public function index() {
		...
		$voteinfo->data->url = NETADDRESS . '/'. $this->module	. '/vote.php?method=index&id=' . $voteinfo->data->id;
		...
		$color = $voteinfo->data->moban ? $voteinfo->data->moban : 'pink';
		
		$this->smarty->assign('id', $id);                  //活动id
		$this->smarty->assign('color', $color);     //模板
	...
	public function cansai() {	
		...
		$color = array('green'=>'#09BB07','blue'=>'#5677FC','orange'=>'#FF9800','pink'=>'#FF6D94','red'=>'#E51C23');
		
		$voteinfo->data->url = NETADDRESS . '/'. $this->module	. '/vote.php?method=index&id=' . $voteinfo->data->id;
		$this->smarty->assign('pid', $pid); 
		$this->smarty->assign('popcolor', $color[$voteinfo->data->moban]); 
		$this->smarty->assign('color', $voteinfo->data->moban);     //模板		
		$this->smarty->assign('voteinfo', $voteinfo->data);//活动内容
	...
	public function signUpHandle() {	
		if($_POST){		
 		  ...
 		  if(!empty($_POST['fileup'])) {
			...
			if($add){
 						$url = NETADDRESS . '/'. $this->module	. '/vote.php?method=success&id=' . $id; 
	...
	public function success() {
		...
		if(empty($id)){
 		...
 		$url = NETADDRESS . '/'. $this->module	. '/vote.php?method=index&id=' . $id;
		$this->smarty->assign('color', $voteinfo->data->moban);     //模板
	...
	public function xiangqing() {
		...
		$voteinfo->data->url = NETADDRESS . '/'. $this->module	. '/vote.php?method=xiangqing&id=' . $id . '&vid='. $vid;
	...
	public function error($msg){
		$index = NETADDRESS . '/'. $this->module .'/vote.php?method=index';

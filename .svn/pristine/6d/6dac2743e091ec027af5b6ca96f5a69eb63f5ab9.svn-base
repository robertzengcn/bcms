...
class AutoMakeHtmlController  extends AutoMakeHtml {
    ...
    public function test() {
		die(json_encode(array('statu' => true)));
	}
   ...
   /**
    ...
    public function reservation( $id ) {
		...
		$fileurl = ABSPATH . '/reservation/' . $id . $this->htmlSuffix;
		if( !file_exists($fileurl) || filemtime($fileurl) < $this->timeout || $this->automake == "dynamic"){
    ...
    public function error() {
		...
		$info = $errorServices->query(array());
		$fileurl = GENERATEPATH . $info->data[0]->page;
		if( !file_exists($fileurl) || filemtime($fileurl) < $this->timeout || $this->automake == "dynamic"){
			...
			$this->smarty->assign("id", $info->data[0]->id);
			$this->smarty->assign('error', $info->data[0]);
			if($this->config['tpl']){
				$this->tagFetch($this->config['tpl'],$fileurl);				
			}else{
				die(json_encode(new Result(false, 35, ErrorMsgs::get(35), Null)));			

...
class CommodityAddons extends Addons {

	public $depr  = '/';	    // 各参数之间的分割符号;
	public $module  = 'addons';	//模块名称
	...
	public function login(){
			...
			$url = NETADDRESS . '/'. $this->module . '/login.php?method=login';
			header("Location:".$url); 
			exit;
	}
	//积分商城首页
	public function index(){
			...
			$pic  =  $this->getServiceMethod('pic','query',array('pid'=>$id->id));
				if(empty($pic->data)){
					$pic->data[0]->pic = UPLOAD . 'commodity/ad1.jpg';
					$pic->data[1]->pic  = UPLOAD . 'commodity/ad2.jpg';
					$pic->data[2]->pic  = UPLOAD . 'commodity/ad3.jpg';
					$pic->data[3]->pic  = UPLOAD . 'commodity/ad4.jpg';
				}
			...
			$this->smarty->display( 'Commodity/home' . TPLSUFFIX );
	}
	...
	public function details(){
		if($_GET['id']){
			...
			if($goods_info->data->pic){
				$goods_info->data->pic = NETADDRESS . '/upload' . $goods_info->data->pic;
			}	
			$this->smarty->assign('piclist', $piclist);	
			$this->smarty->assign('info', $goods_info);			
			$this->smarty->display( 'Commodity/info' . TPLSUFFIX );
		}
	}
	//搜索 商品列表
	 ...
	 //购物车
		public function cart(){
			...
			if(count($shoplists)>=1){
				foreach($Mcartlist as $key => $val){
						...
						if(isset($shoplists[$key])){
							...
							$shoplists[$key]['pic'] = NETADDRESS . '/upload' . $shoplists[$key]['pic'];							
						}
				}
			...
			$CartCount = count($Mcartlist);	
				...
				$this->smarty->display( 'Commodity/shopcart' . TPLSUFFIX );			
		}
		//成功页面
	public function success(){
		$this->_InjectPublicData();
		$this->smarty->display( 'Commodity/success' . TPLSUFFIX );		
	}
		//订单管理
	public function order(){
		...
		if($memberId){
			...
			$this->smarty->display( 'Commodity/order' . TPLSUFFIX );				
		}else{
			$this->login();
		}
	...
	public function points(){
		...
		if($memberid){
			...
			$this->smarty->display( 'Commodity/points' . TPLSUFFIX );	
		}else{
			$this->login();
		}
	}
	...
	public function code(){
		...
		if($memberid && $order){
			...
			$this->smarty->assign('buylist', $buylist);				
			$this->smarty->display( 'Commodity/barcode' . TPLSUFFIX );	
		}		
	}
	//用户换购状态
	...
	public function _InjectPublicData() {
		...
		if($goods_logo->data){
			...
			$goods_url[0] = UPLOAD . 'commodity/shop_logo.png';
			$goods_url[1] = UPLOAD . 'commodity/shop_icon_32.png';
			$goods_url[2] = UPLOAD . 'commodity/shop_icon_72.png';			
		}
		/** 注入nav **/
		if($this->initlogin){
		...
		/** 注入轮播图片 **/		
		/** 注入url **/
	    $commodity = NETADDRESS . '/'. $this->module .'/commodity.php';
 ...
 */
	public function _Url($url='',$vars='',$suffix=false,$domain=true){
		...
		if($domain){
			$domain = NETADDRESS . $this->depr. $this->module . $this->depr . $path[0] . ".php?method=" . $path[1];			
		}else{
			$domain = $this->module . $this->depr . $path[0] . ".php?method=" . $path[1];			
		}
		$con = '';
        if(!empty($vars)) { // 添加参数

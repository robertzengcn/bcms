...
class TagLib {
    ...
    public function __construct() {
        ...
        $this->tpl     = Tags::instance('Template');
    }
    ...
    public function parseCondition($condition) {
        ...
        if(false !== strpos($condition, '$boyi'))
            $condition      =   preg_replace('/(\$boyi.*?)\s/ies',"\$this->parseVar('\\1');" , $condition);        
        return $condition;
    }
    ...
    public function parseVar($varStr){
        ...
        if(count($vars)>=3){
            ...
            switch($vars[1]){
                ...
                case 'VERSION':  $parseStr = 'BOYI_VERSION';break;

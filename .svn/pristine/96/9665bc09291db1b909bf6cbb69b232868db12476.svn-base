...
class dloginAddons extends Addons {
	...
	public function loginout(){
		session_destroy();
		header("Location: /Addons/dlogin.php");
	}
	//登录
	public function login(){
		$this->_publicData();	
		$this->smarty->display( 'Draw/login' . TPLSUFFIX );			
	}
	//注册
	public function register(){
		$this->_publicData();	
		$this->smarty->display( 'Draw/reg' . TPLSUFFIX );			
	}
	//获取手机注册验证码
	...
	public function userLogin(){
		...
		if($result->statu){
			...
			$data['status'] = 1;
			$data['info'] = "登录成功";
			$data['jump'] = NETADDRESS . '/addons/draw.php?method=index';
		}else{				
			$data['info'] = "账号或密码错误";
		}
			echo json_encode($data);		
	...
	public function checkReg(){
		...
		if($obj->statu){
			$result['status'] = 1;
			$result['info'] = "注册用户成功";
			$result['jump'] = NETADDRESS . '/addons/draw.php?method=index';
		}else{
			$result['status'] = 0;
			$result['info'] = $obj->msg;			
		}
	...
	private function _publicData() {
		...
		/** 注入url **/
	    $login = NETADDRESS . '/addons/dlogin.php';

function Reservation(){
	...
	this.getReserDotor=function(){
		$("#TotalDoctorlist").grid({
			...
			aoColumnDefs : [
				{sClass:'text-c',aTargets:[0]},{sClass:'text-c',aTargets:[1]},{sClass:'text-c',aTargets:[2]},{sClass:'text-c',aTargets:[3]},{sClass:'text-c',aTargets:[4]},{sClass:'text-l',aTargets:[5]},{sClass:'text-c',aTargets:[6]},{sClass:'text-c',aTargets:[7]},{sClass:'text-c',aTargets:[8]},{sClass:'text-c',aTargets:[9]}
							],
			field : [
						{
				            	data : 'id',
				            ...
				            { data: 'description',render:function(value){return value.substr(0,20)+'...';}},	
							{ data: 'rel_name',render:function(value){var str=(value=='未绑定')?'<span class="label label-warning radius">'+value+'</span>':'<span class="label label-success radius">'+value+'</span>'; return str;}},	
							{ data: 'resernum'},
							{ data: 'mark'},
				            { data : 'id',
							  render : function(value,type,row){								
								  var str = '';
								  str += '<a class="ml-5 editdoctor" title="编辑" style="text-decoration:none" onclick="doctormanager.openEditWID(\'编辑医生信息\',\'edit-doctor.html?id='+value+'\',\'900\',\'450\')"><i class="Hui-iconfont">&#xe6df;</i></a><a title="关联形象展示医生" class="ml-5"  onclick="doctormanager.openEditWID(\'关联形象展示医生\',\'doctor-to-doctor.html?id='+value+'&relid='+row.rel_id+'\',\'900\',\'450\')" style="text-decoration:none"><i class="Hui-iconfont">&#xe60e;</i></a><a title="删除" class="ml-5"  onclick="doctormanager.delDoctor('+value+')" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>';
						    	  return str;
							  },
							  orderable:false
							}
		...
		$(function(){
			...
			self.cmd(gHttp,{controller:'Doctor',method:'query'},function(re){
				...
				$.each(re.rows, function(idx, obj) {
					if(para.relid!=undefined&&obj.id==para.relid){
						str+='<li class="clinic_item radio-box"><input type="radio" value="'+obj.id+'" name="doctorother" checked="checked"/>&nbsp;'+obj.name+'</li>'; 
					}else{
	...
	this.statBySchedule = function() {
		...
		self.cmd(gHttp,{controller:'ResVation',method:'getStatData',type:'1',start:start,end:end}, function(result) {
			...
			var str="<tr><td  colspan='8' class='text-r'>已约号源：<span class='label label-success radius'>"+total.about+"</span>（个）&nbsp;&nbsp;未约号源：<span class='label label-success radius'>"+total.no+"</span>（个）&nbsp;&nbsp;总号源：<span class='label label-success radius'>"+total.total+"</span>（个）</td></tr>";
			$('#listAll1 tbody').append(str);
			//拼接统计标题
			var title_str=start+'至'+end+'排班号源(已约/未约)周分布统计';	
			//格式化数据
			var stype = $("#showType").val();
			var newsdata = (stype=='pie') ? total : data[3];
			var statData = self.formatStatData(newsdata,'1');
			//显示统计图
			self.showStatImg(statData, 'statImg1', '号源(个)', title_str,'',stype);
		});
	}
	/**
	 * 到诊率统计
	...
	this.statByArrive = function() {
		var start = $('#start').val();
		var end = $('#end').val();
		self.cmd(gHttp,{controller:'ResVation',method:'getStatData',type:'2',flag:'department',start:start,end:end}, function(result) {
			var ttl = result.ttl;
			var colspan = ttl+2;
			var data = result.rows;
			var total = result.total;
			var tdWidth = 120;
			var header='<tr class="text-c"><th width="100">统计项目</th>';
			var arrive='<tr><td class="text-c">到诊（人）</td>';			
			var about='<tr><td class="text-c">已约（人）</td>';
			var percentage='<tr><td class="text-c">到诊率（%）</td>';
			$.each(data,function(i,v){
				header+='<th width="'+tdWidth+'">'+v.department_name+'</th>';
				arrive+='<td class="text-c">'+v.arrive_num+'</td>';
				about+='<td class="text-c">'+v.about_num+'</td>';
				percentage+='<td class="text-c">'+v.percent+'</td>';				
			});
			header+='</tr>';
			var tbodyer=arrive+'</tr>'+about+'</tr>'+percentage+'</tr>';		
			$('#listAll2').find('thead').html(header);
			$('#listAll2').find('tbody').html(tbodyer);
			var str="<tr><td  colspan='"+colspan+"' class='text-l'>已到诊总人数：<span class='label label-success radius'>"+total.arrive+"</span>（人）&nbsp;&nbsp;已约总人数：<span class='label label-success radius'>"+total.about+"</span>（人）&nbsp;&nbsp;到诊率：<span class='label label-success radius'>"+total.percent+"</span>（%）</td></tr>";
			$('#listAll2 tbody').append(str);
			//表格滚动处理
			self.scrollTab($('#info2'),tdWidth,100,$('#li_stat_2'));
			//格式化数据
			var statData = self.formatStatData(data,'2','department');
			//显示统计图
			var stype = $("#showType").val();
			//拼接统计标题
			if(stype=='pie'){
				var title_str=start+'至'+end+'到诊人数与已约人数比例统计';				
			}else{
				var title_str=start+'至'+end+'各科室到诊率统计';
			}
			self.showStatImg(statData, 'statImg2', '人次', title_str,'',stype);
		});		
	}
	...
	this.updateStatData = function() {
		var start = $("#start").val();
		var end = $("#end").val();
		self.cmd(gHttp,{controller:'ResVation',method:'updateStatData', start:start, end:end},function(ret){
			if(ret.statu){

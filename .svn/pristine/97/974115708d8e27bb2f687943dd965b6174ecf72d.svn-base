...
//		    }
	...
	this.initStatItems = function() {	
		self.checkAndroidOrIos();
		$("#statItems li").click(function(){
			var type = $(this).attr("flag");
			window.open('patientlist.html?type=' + type ,'_self');
		});
...
/* 		var url = {controller:'Patient',method:'getStatData',type_id:2,type:'1',start:start,end:end};
		self.cmd(gHttp,url, function(result) {
			...
			$.each(data, function(k,v){
					...
					html += '<td>' + v.disease_name + '</td>';
					for(var i=1;i<10;i++){
						html += '<td>' + v[i*10].replace(/<b>\/<\/b>/,'').replace(/&nbsp;&nbsp;/,'<br/>') + '</td>';					
					}
					html += '</tr>';
			});
			$("#listAll1_2 tbody").html(html);
		}); */
	}
	...
	this.statByReceptionist = function() {
		...
		self.cmd(gHttp,{controller:'Patient',method:'getStatData',type:'3',start:start,end:end}, function(result) {
			...
			var tbody = '';
			$.each(data,function(i,v){
				tbody += '<tr "text-c" class="am-primary">';
	...
	this.statByDoctor = function() {
		...
		var page = 0;
		//上拉加载
		var header ='<tr class="text-c"><th width="90">姓名</th>';
		...
		$('#listAll5').dropload({
		    ...
		    loadDownFn : function(me){
		    	...
		    	self.cmd(gHttp,url,function(result) {
					...
					var tbody ='';
					$.each(data,function(i,v){
						tbody +='<tr class="text-c am-primary">';
		...
		$('#listAll8').dropload({
		    ...
		    loadDownFn : function(me){
		    	...
		    	self.cmd(gHttp,url,function(result) {
					...
					//显示统计图
					self.showStatImg(statData, 'statImg8', '就诊人数', '按诊疗项目统计','','bar');

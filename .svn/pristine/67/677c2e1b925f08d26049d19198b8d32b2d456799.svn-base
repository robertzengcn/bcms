...
class Ask extends InterfaceAbstract {
	...
	protected function __getList(){
		if( isset( $_REQUEST['page'] ) && isset( $_REQUEST['row'] )  ){
		    ...
		    $this->msgPut(false, '缺少page或row参数.', null, 1);
		}
	}
	...
	protected function __getInfo(){
		if( isset( $_REQUEST['question_id'] ) && (int)$_REQUEST['question_id'] >= 1 ){
			...
			if( count( $result ) <= 0 ){
			    $this->msgPut(false, '不存在对应的问题信息.', null, 1);
			}else{
				$data = array();
				$data['code'] 		   = 0;
				$data['msg']  		   = 'success';
		    ...
		    $this->msgPut(false, '缺少question_id或参数不正确.', null, 1);
		}
	}
	/**
	 * 回答问题
	...
	protected function __answer(){
		...
		if( isset( $_REQUEST['question_id'] ) ){
			...
			if( count( $result ) <= 0 ){
				...
				if( $content == '' ){
				    $this->msgPut(false, '回复内容不能为空.', null, 1);
				}
				$isAdd = false;
		    ...
		    $this->msgPut(false, '缺少question_id或缺少content参数.', null, 1);
		}
	}

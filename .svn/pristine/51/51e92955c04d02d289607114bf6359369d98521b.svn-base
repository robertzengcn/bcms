...
class OrdercommodityService extends BaseService {

    /**
     * 构造函数
     */
    public function __construct() {
        $this->dao = new OrdercommodityDAO();
    }
...
*/
    ...
    public function addorder($arr){
    	try{	
    		$this->dao->beginTrans();
    $order=new Ordercommodity();
    $order->order_id=$arr['order_id'];
    ...
    public function getorderdetail($orderid){
    	...
    	$commodityarr=$this->dao->getorderdetail($arr);
    	$commodityser=new CommodityService();
    	foreach($commodityarr as $key=>$val){
    		
    		$commodityobj=$commodityser->getcombyid(array('id'=>$val->commodity_id));
    		
    		if($commodityobj->data){
    		$commodityarr[$key]->pic=$commodityobj->data->pic;
    		}
    	}
    	return $commodityarr;
    }

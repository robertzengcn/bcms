...
/* 			  #Keywords
			  ...
			  if (empty($matches[1])) {
				...
				$meta['description'] = (!empty($matches[1])) ? $matches[1] : '绍兴市上虞区第三医院是一家以康复医学为特色，集医疗、预防、保健、科研于一体的二级综合性医院...';
			   #img
			  preg_match( '/<img.+src=\"?(.+\.(jpg|gif|bmp|bnp|png))\"?.+ data-share=\"share\">/si', $data, $matches );
				$meta['picurl'] = (!empty($matches[1])) ? $matches[1] : 'http://new.82233333.com/upload/picmanager/20170114180652726.jpg';
		 }
      return $meta;
	}
...
/* 		$filename = ABSPATH.'/weixindb/weixinApp.json';	
		...
		$appId = 'wxe12e2f72f68905c9';
		$appsecret = 'f6c7530fe18f6738c0ce78f3b90dc2f5';	 //要绑定JS接口安全域名
		...
		if($jsapi_ticket==''){
			echo json_encode(new Result(false, 0, 'invalid appsecret', ''));
			exit;
		}
	...
	private function make_ticket($appId,$appsecret){
		...
		$data1 = json_decode(file_get_contents($tokenurl));			
		if ($data1=="" || $data1->expire_time < time()) {	

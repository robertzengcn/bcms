...
class Register extends Mod{
	...
	public function index() {
        ...
        session_start();
        $this->smarty->assign("SEO_TITLE", "用户注册");        
	...
	public function findbackpass(){
		...
		$_SESSION['token']=md5(time().rand(1,999));//产生一个随机数防止域外提交
		$this->smarty->assign("token", $_SESSION['token']);
		$this->smarty->assign("SEO_TITLE", "找回密码");
...
/* 		if(!isset($_REQUEST['auto_code'])||md5($_REQUEST['auto_code'])!=$_SESSION['MOBILE_AUTH_CODE']){
	...
	/**
	 * 验证cms短信验证码，把验证成功的手机号记录缓存
	 * @param $_Request['mobile]
	 * */
   public function validcmscode(){
   	$code=getRequest('code',null);
   	if(!$code){
   		$this->msgPut(false,11,'缺少短信验证码参数',null);
   	}
 	
   	if(!session_id()) session_start();
   	if(!$_SESSION['MOBILE_AUTH_CODE']){
   		$this->msgPut(false,15,'短信验证码不正确',null);
   	}
   	if(md5($code)== $_SESSION['MOBILE_AUTH_CODE']){
   		
   		if(isset($_SESSION['validmobile'])){
   			array_push($_SESSION['validmobile'],$_SESSION['MOBILE_AUTH']);
   		}else{
   		...
   		$this->msgPut(false,11,'短信验证码错误',null);
   	}

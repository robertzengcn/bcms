...
// 		$result = $reser->getDetail($_REQUEST);
	...
	public function saveBookingInfo() {
	    	...
	    	if($needmsgres->val){//如果需要短信验证
	    		...
	    		//array_push($_SESSION['validmobile'],$_REQUEST['telephone']);//测试时打开，测试后关闭
    	if(isset($_SESSION['validmobile'])){
    		...
    		if(!in_array($mobile, $_SESSION['validmobile'])){
    			$this->msgPut(false,12,'手机号没有验证过',null);
    		}
	   ...
	   $bookservice=new ResBookingInfoService();
		...
		if($result['ttl']!=0){
			$rows=$result['rows'][0];
			$this->msgPut(false,18,'您己经预约过'.$rows->date.' '.$rows->doctor_name.'的号，不能重复预约，若您约错号，请进入您的个人信息中心在我的预约中取消预约！',null);
		}else{
			...
			if(isset($_REQUEST['resvationID'])&&$_REQUEST['resvationID']){
				...
				$this->msgPut(false,19,'缺少resvationID参数',null);
			}	
							
			if(!isset($_REQUEST['times'])||strlen($_REQUEST['times'])<0){
				$this->msgPut(false,15,'缺少times参数',null);											
			}else{
...
//     	$docs->data = array_slice($docs->data,($page-1)*$where['size'],$where['size']);
    ...
    public function getmyreser(){
    	...
    	$reserdetailser=new ResBookingInfoService();
    	
    	if(isset($_REQUEST['page'])&&isset($_REQUEST['size'])){
    		$result=$reserdetailser->query(array('telephone'=>$_SESSION['user'],'page'=>(int)$_REQUEST['page'],'size'=>(int)$_REQUEST['size']));
    	}else{
    		$result=$reserdetailser->query(array('telephone'=>$_SESSION['user']));
    	}
    	$ttl=$reserdetailser->totalRows(array('telephone'=>$_SESSION['user']));
    ...
    if(isset($_REQUEST['start'])&&isset($_REQUEST['end'])){
    	...
    	if(!empty($result)){
    		...
    		echo json_encode(array('statu'=>true,'msg'=>null,'code'=>0,'data'=>$result));
    		exit();
    	}else{
    		echo json_encode(array('statu'=>false,'msg'=>'当前医生本周没有排班','code'=>15,'data'=>null));
    		exit();
    	}

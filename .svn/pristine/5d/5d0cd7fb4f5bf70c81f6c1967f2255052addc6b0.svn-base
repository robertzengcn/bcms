...
class ArticleService extends BaseService {
    ...
    public function query($where, $flag = '') {
        ...
        foreach ($articles as $key => $value) {
			if($flag !='makehtml'){
				$value->author = $workerDAO->getName($value->worker_id);
				$value->doctor_name = $doctorDao->getName($value->doctor_id);
				$value->disease_name = $diseaseDao->getName($value->disease_id);
				$value->department_name = $departmentsDao->getName($value->department_id);
			}
        	$dep_url = $departmentsDao->getUrlById($value->department_id);
    ...
    public function  isTrueByArticleId($id, $department, $disease){
         return $this->dao->isTrueByArticleId($id, $department, $disease);
    }   
    ...
    public function getuniquetitle(){
    	return $this->dao->getuniquetitle();
    }
}

	...
	this.initEdit = function() {
		$(function(){
			...
			$('body').on('click','#save',function(){
				var phonelist='';
				var linum=$('.selectList li').length;
				var i=1;
				$('#errormsg').html('');
				if(linum==0){
					$('#errormsg').html('<span class="Validform_checktip Validform_wrong">收件人不能为空！</span>');
				}else{
					$('.selectList li').each(function(){
						if($(this).attr('ptelephone')!=''){
							if(i<linum){
								phonelist+=$(this).attr('ptelephone')+',';
							}else{
								phonelist+=$(this).attr('ptelephone');
							}
							i++;							
					...
					$('#formEdit').checkAndSubmit('save',{controller:'Wuxian',method:'sendmessage',mobile:phonelist},function(rel){
						if(rel.statu){					

...
class SuccessTag extends CommonTag {
	 ...
	 */
	public function getByRecommend($recommend,$name='success_id',$is_tomobile=''){
	 ...
	 * @param unknown_type $recommend_id
	 */
	public function getByRecommendDep($recommend,$dep_id,$name='success_id',$is_tomobile=''){
	    if (!is_numeric($recommend)){
	        $position = new RecommendPositionService();
	        $result = $position->getByName($recommend,$is_tomobile);
	        $recommend_id = $result->id;
     ...
     * 获取列表 
     */
    public function getSucList($sql){

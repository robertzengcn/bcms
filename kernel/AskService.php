...
class AskService extends BaseService {
     ...
     * 回答问题开启极光推送
     * @param $alis用户别名
     * @param $question_id问题id
     * @param $answered 回答类型 1为首次回答  0为再次回答
     * */
    public function askjspush($alis,$question_id,$answered=1){
    	...
    	if($jpushstate){//如果推送开启
    		...
    		$jpush_apnsdata=$contactser->getContact('jpush_apns')->data;
    		...
    		if($jpush_apnsdata->val){//1为线上,0为线下
    			$jpush_apns=true;
    		}else{
    			$jpush_apns=false;
    		}
    		...
    		try{
    			...
    			$jpushser->addAlias($alis);
    			//$jpushser->addAlias('341');
    			$jpushser->setNotificationAlert('你的提问有了新回答');
    			...
    			$jpushser->options(array('apns_production'=>$jpush_apns));
  
    			$jpushser->send();
    			$error_log=GENERATEPATH . 'log/jpush_error.log';
    			error_log(date('Y-m-d H:i:s') . ' JPUSH success ' .$alis. PHP_EOL, 3, $error_log);
    		}catch(Exception $e){
    			$error_log=GENERATEPATH . 'log/jpush_error.log';

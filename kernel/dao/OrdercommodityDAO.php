 ...
 *
 */
class OrdercommodityDAO extends DBBaseDAO {

    public function __construct() {
        parent::__construct();
        $this->tableName = TABLENAME_ORDERCOMMODITY;
    }
...
//     	}
    ...
    public function getorderdetail($arr){
    	...
    	$result=R::findAll($this->tableName,'order_id=:order_id',$arr);
    	//$result=R::getAll("select o.*,co.pic from `ordercommodity` as o LEFT JOIN `commodity` as co ON o.commodity_id=co.id where o.order_id=:order_id");
        $array = array();
    	$class = ORMMap::$classes[$this->tableName];
    	foreach ($result as $k => $bean) {
    		$entity = new $class();

...
class ResTemplateService extends BaseService {

    public function __construct() {
        $this->dao = new ResTemplateDAO();
    }
   ...
   public function modTemplate() {
   	...
   	if($_REQUEST['plushtime']==''||!isset($_REQUEST['plushtime'])){
   		$restemplate->plushtime=time();
   	}
   	$restemplate->validate();
   	$re = $this->dao->modTemplate($restemplate);
   	if($re){
    ...
    public function query($where) {
        ...
        foreach ($reservations as $k => $v) {        	
        	...
        	$reser[$k]['description']=$v->description;
        	$reser[$k]['plushtime']=date("Y-m-d", $v->plushtime);
        }
    ...
    public function deleteBatch($ids) {    	
    	$resschedule=new ResScheduleDAO();
    	foreach($ids as $v){
    		$arr['tem_id']=$v;
    		$resschedule->deleteAll($arr);
    	}
        Entity::isIds($ids);
        $beans = $this->dao->getBatch($ids);
        return $this->dao->deleteBeans($beans);
        //根据模板ID，同时清除联带的医生排班数据
	 ...
	 * 通过department_id获取id
	 * */
	public function getIdByDepartmentName($department_name){
			return $this->dao->getId($department_name);		
	}
}
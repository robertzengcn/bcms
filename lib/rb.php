...
interface RedBean_QueryWriter
{
	 ...
	 * @return array
	 */
	public function queryRecord( $type, $conditions = array(), $addSql = NULL, $bindings = array(), $field = NULL );
...
abstract class RedBean_QueryWriter_AQueryWriter { //bracket must be here - otherwise coverage software does not understand.
	...
	public function queryRecord( $type, $conditions = array(), $addSql = NULL, $bindings = array(),$field = NULL )
	{
		...
		$field = ($field && is_string($field))?$field:'*';
		$table = $this->esc( $type );

		$sql   = $this->makeSQLFromConditions( $conditions, $bindings, $addSql );
		$sql   = "SELECT {$field} FROM {$table} {$sql} -- keep-cache";
...
class RedBean_OODB extends RedBean_Observable
{
	 ...
	 * @throws RedBean_Exception_SQL
	 */
	public function find( $type, $conditions = array(), $sql = NULL, $bindings = array(),$field = NULL )
	{
		...
		try {
			...
			$beans = $this->convertToBeans( $type, $this->writer->queryRecord( $type, $conditions, $sql, $bindings,$field ) );
...
class RedBean_Finder
{
	 ...
	 * @throws RedBean_Exception_Security
	 */
	public function find( $type, $sql = NULL, $bindings = array(), $field=NUll)
	{
		if ( $sql instanceof RedBean_SQLHelper ) {
			list( $sql, $bindings ) = $sql->getQuery();
		}
		...
		return $this->redbean->find( $type, array(), $sql, $bindings, $field);
	}
...
class RedBean_Facade
{
	...
	*/
	 ...
	 * @return array
	 */
	public static function find( $type, $sql = NULL, $bindings = array(), $field=NULL )
	{
		
		return self::$finder->find( $type, $sql, $bindings, $field );
	}

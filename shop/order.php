...
if(isset($_REQUEST['size'])&&strlen($_REQUEST['size'])>0){
	...
	if(isset($_REQUEST['page'])&&isset($_REQUEST['size'])){
		$param=array('member_id'=>$_SESSION['member_id'],'page'=>$page,'size'=>$size,'order'=>'date desc');
	}else{
		$param=array('member_id'=>$_SESSION['member_id'],'order'=>'date desc');
	}
...
if(isset($_REQUEST['order_stute'])&&strlen($_REQUEST['order_stute'])>0){
	...
	public function checkout(){
		...
		if(!isset($_REQUEST['cart_id'])&&strlen($_REQUEST['cart_id'])<=0){
			echo json_encode(array('statu'=>false, 'code'=>9958,'msg'=>'缺少参数 cart_id','data'=>null));
			exit();
		}
		...
		if(!empty($data)){
			...
			$pricetotal=0;
			foreach ($data as $k=>$v){
				$num=$this->getServiceMethod('CommodityCart','count_score',array('member_id'=>$_SESSION['member_id'],'id'=>$v));
				$price=$this->getServiceMethod('CommodityCart','count_price',array('member_id'=>$_SESSION['member_id'],'id'=>$v));				
			...
			foreach ($data as $key=>$val){
				...
				$cart = $this->getServiceMethod('CommodityCart','getmebercart',array('member_id'=>$_SESSION['member_id'],'id'=>$val));
				...
				//if($result->data->quantity>0&&$cart->data['quantity']<=$result->data->quantity){
					...
					if($orderdetail->statu){
						...
						$this->getServiceMethod('CommodityScorereduce','addlog',array('uid'=>$_SESSION['member_id'],'score'=>$cart->data['final_score'],'note'=>'购买商品消费积分'));
						$this->getServiceMethod('CommodityCart','deleteBatch',array('id'=>$val));
						$cnum++;

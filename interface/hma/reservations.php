...
class Reservations extends InterfaceAbstract {
	public function __construct() {
	    /*
	    $_REQUEST['method'] = isset($_REQUEST['part']) ? $_REQUEST['part'] : '';
	    $_REQUEST['time'] = time();
	...
	private function get_cancel_reservation(){
		...
		if(isset($_REQUEST['id'])&&strlen($_REQUEST['id'])>0){
			...
			$this->msgPut(true, '您的预约已成功取消!', null);
		}else{
			$this->msgPut(false, '缺少id', null, 2);
		}
	}
	...
	private function get_appointment_list(){		
		...
		$arr=array('member_id'=>$_SESSION['member_id'], 'telephone'=>$_SESSION['member_mobile']);		
		...
		if(!empty($result)){
			...
			$this->msgPut(false, '未找到与您相关的预约信息！', null);
		}	
	}
	...
	private function get_appointment_finish(){		
        ...
        if(!$boolcheck&&$bookcheck){
        	...
        	$this->msgPut(false, '该时间点已有约，请更换', null);
        }
	...
	private function queryRes($arr=array()) {
		...
		if( is_object( $find ) ) {
			...
			if( ($bookingNum-(int)$find->mark) < 1 ) {
				$this->msgPut(false, '该预约时间段已经无号了', null);
			}else{
				return $find;
			}
		}else{
			$this->msgPut(false, 'time_id对应的预约信息不存在', null);
		}
	}
	...
	private function checkrepeatbook($arr){
		...
		if($result['ttl']!=0){
			$rows=$result['rows'][0];
			$this->msgPut(false, '您己经预约过'.$rows->date.' '.$rows->doctor_name.'的号，不能重复预约，若您约错号，请进入您的个人信息中心在我的预约中取消预约！', null);				
		}else{			
			return true;
		}
	}
	...
	private function runRes( $resInfo,$arr ) {	
		...
		if (class_exists('ClientService')) {//判断client表里是否存在这个用户
			...
			$resresult=$clientser->getclientbytn($arr['telephone'],$arr['username'])->data;
			if(!$resresult->id){//如果client表里不存在
				$client=new Client();
				$client->username=$arr['username'];
				$client->gender=(int)$arr['sex'];
				$client->age=(int)$arr['age'];
				$client->address=$arr['address'];
				$client->telephone=$arr['telephone'];
	...
	* */
	protected function get_resnum(){
	    //time与验证时的time冲突，故将time改为time_field		
		if(isset($_REQUEST['id'])&&strlen($_REQUEST['id'])>0){
			if(isset($_REQUEST['time_field'])&&$_REQUEST['time_field']&&isset($_REQUEST['date'])&&$_REQUEST['date']){
				$id=(int)$_REQUEST['id'];				
				$times=strtotime($_REQUEST['date'].' '.$_REQUEST['time_field']);				
